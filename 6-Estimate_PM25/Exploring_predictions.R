...
for(i in 1:11){
  nearest_ind<- get.knnx(input_locs, Stat[which((Stat$State == State[i])&(Stat$County_FIPS == LOIs[i,2])&
                                    is.na(Stat$Tract_code) & is.na(Stat$ZCTA5_code)),c("Lon", "Lat")], k = 3)$nn.index
...
for(s in State[-8]){
  ...
  these_obs<- DF_no_CMAQ[which((DF_no_CMAQ$Lon == lons[which(State == s)])&(DF_no_CMAQ$Lat == lats[which(State == s)])), c("PM2.5_Obs", "Date")]
  
  png(filename = paste0("~/Plots/", county_of_interest[1,1],"_", s, "_time-series_2.png"), width = 960)
