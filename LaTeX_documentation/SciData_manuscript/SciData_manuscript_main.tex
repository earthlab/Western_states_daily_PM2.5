...
[insert Figure 1:  monitor locations (points) and state boundaries] % Generated using  Map_Monitor_Locations.R
...
more rural areas \cite{EPANPM25IMPROVE2017} for the 11-state region (Figure \ref{fig:Map11States}) including any of the following parameter codes: 88101, 88500, 88502, 81104 \cite{EPANPM25Memo2017,EPANPM25Parameters2017,EPANAllParameters2017}. We also retrieved all available PM\textsubscript{2.5} data in the Fire Cache Smoke Monitor Archive (\url{https://wrcc.dri.edu/cgi-bin/smoke.pl}), which includes U.S. Forest Service monitors that were deployed to capture air quality impacts from wildfires. 
...
%Colleen, we should put in a bit more citations about how the MAIAC has been shown to be a good predictor in previous studies. 
...
%Melissa and Ellen, I found that the documentation for the NLCD had two different buffer sizes listed - do you know if you used the one above, or these ones:  1km, 5km, and 10km radius buffers using planar buffering.
...
%Colleen needs to put in citations about why we added in NDVI
%Ellen and Melissa, do you know if we got this measure monthly or less frequently?
...
%Ellen - could you provide more information on what types A and C are?

To account for seasonality in PM\textsubscript{2.5} data, we created a predictor variable to be the cosine of day-of-year. We also created dummy variables for each state and month in our study domain to allow for spatial and temporal variation in the data that could not be explained by %nany 
any 
of the other spatial, temporal, or spatiotemporal variables. 

%Melissa, have I put in all of the data sources?
