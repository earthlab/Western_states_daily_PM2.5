...
\author[mysecondaryaddress]{M.M. Maestas} % move me to 1st author
%\ead[url]{www.elsevier.com}
...
% this paper just machine learning - no camx, not focused on fires
% important improvement - focus on western US - which has been poorly estimated in national - using deep blue instead of dark target, including MAIAC (finer spatial resolution) more monitoring data - fire cache, states, relationships between variables depend on spatial and temporal scales - better estimates for western US

% also consider ES&T journal or 
% Environmental Pollution or Science of the total environtment (also consider for kriging paper)

% put some of the intro information into review paper
...
\section{Materials and Methods} % about 1300 words (distributed below); can consider putting some in supplemental materials everything we need to say won't fit here.
...
\subsection{Setting} % about 130 words
[11 western US states, 2008-2014]

[1 short paragraph]

% [include map of western US with monitor site locations, etc.]
\begin{figure} 
\centering 
...
\subsection{Training Data} % about 130 words
...
\subsection{Predictor Variables} % see word count distribution below

[Include large Table cataloging the predictor variables]

\subsubsection{CAMx model - remove section} % about 150 words
...
\subsubsection{Remote Sensing data} % about 150 words; currently:
...
\subsubsection{Other Earth Data} % about 150 words
...
\subsection{Machine Learning Model/Statistical Analysis} % see word count distribution below

	\subsection{Total PM2.5} % about 260 words
...
We will set aside a portion of the PM\textsubscript{2.5} monitoring data as a validation data set that will not be used in any of the folds of training data in the machine learning algorithm. We will also calculate the RMSE that compares the validation data with the corresponding estimates from the optimal algorithm. The CV-RMSE of the model gives us a sense of the uncertainty in the model estimates that will then be used in the epidemiological analysis to adjust for the error in using this model to estimate exposures (see below).
	...
	\subsection{Attribution of Total PM2.5 to Wilfires, Prescribed Fires, and Urban Sources} % about 130 words
...
\subsection{model validation, handling uncertainties, etc.} % about 200 words
...
%%%% Pick up incorporating text from here %%%%
