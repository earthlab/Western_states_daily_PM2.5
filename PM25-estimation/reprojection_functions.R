...
reproject_monitors.fn <- function(this_source_file) {
...
monitors<- read.csv(file.path(ProcessedData.directory,this_source_file), stringsAsFactors = FALSE) # load data
...
# NAD83 is the datum we're converting to, so not much processing neede for this
DF83<- df83[,1:2]

nad83_vec <- data.frame(matrix("NAD83",nrow=dim(df84)[1],ncol=1)) # create vector indicating NAD83 (new datum for all)
table_84<- cbind(coordinates(df84), df84$Datum, coordinates(DF84), nad83_vec)
colnames(table_84)<- c("old_lon", "old_lat", "old_Datum", "Lon", "Lat","Datum")
