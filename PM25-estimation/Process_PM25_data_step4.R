...
#all_combined <- rbind(part_b_loc,part_d_loc)
#no_dup <- all_combined[!duplicated(all_combined),]
#names(three_cols_data) <- c("Latitude","Longitude")#,"Datum")


#sqldf('select * from part_d_loc except * from part_b_loc')

#part_d_not_in_b_loc_step1 <- rbind(part_d_loc, part_b_loc)
#x <- rbind(part_d_loc[ , c("Lat", "Lon", "Datum","Easting","Northing")], part_b_loc[ , c("Lat", "Lon", "Datum","Easting","Northing")])
#part_d_not_in_b_loc <- x[! duplicated(x, fromLast = TRUE) & seq(nrow(x)) <= nrow(part_d_loc), ]
