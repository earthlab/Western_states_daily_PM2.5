fill_in_aves_coloc_unique_PC_POC_MN.fn <- function(this_day_all_data_in,input_mat3_aves,rstart_aves,input_mat3_colocated,rstart_colocated,lat_tolerance_threshold,lon_tolerance_threshold) {
  ...
  if (length(unique(this_day_all_data$Datum))>1) { # check that datums match
    ...
    } else { # if (these_datums[1]=="NAD83" & these_datums[2]=="WGS84" & length(unique(this_day_all_data$PM2.5_Lat))==1 & length(unique(this_day_all_data$PM2.5_Lon))==1 ) {
      stop("Datums don't match. Look at data/code and write more code")
    } # if (these_datums[1]=="NAD83" & these_datums[2]=="WGS84" & length(unique(this_day_all_data$PM2.5_Lat))==1 & length(unique(this_day_all_data$PM2.5_Lon))==1 ) {
    } else if (length(unique(this_day_all_data$Datum))==1) { # only one datum value# if (length(unique(this_day_all_data$Datum))>1) { # check that datums match
      print(paste("Datum values are identical: ",unique(this_day_all_data$Datum),sep = ""))
      input_mat3_aves[rstart_aves:rstop_aves,c("Datum")] <- unique(this_day_all_data$Datum) # input composite of data
    } else {# if (length(unique(this_day_all_data$Datum))>1) { # check that datums match
