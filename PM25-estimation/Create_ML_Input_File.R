...
FMLEdata_all_states <- read.csv(file.path(FMLE.directory,this_source_file), header = T, sep = ",",blank.lines.skip = F)
# load parameter description
FMLEdata_Parameter_MetaData <- read.csv(file.path(FMLE.directory,this_source_file_full), header = T, sep = ",",blank.lines.skip = T,nrows = 1,skip = 242)
rm(this_source_file_full)
...
# "PM2.5_Obs"   
input_mat1[row_start:row_stop,c("PM2.5_Obs")] <- as.numeric(FMLE_StudyStates_sepCodes[,c("RCFM.Val")]) #("MF.Val")])

# "PM25_Station_Name" 
input_mat1[row_start:row_stop,c("PM25_Station_Name")] <- as.character(paste(FMLE_StudyStates_sepCodes[,c("SiteName")],FMLE_StudyStates_sepCodes[,c("SiteCode")])) #"SiteName" "SiteCode"

# "Data_Source_Name_Display"
input_mat1[row_start:row_stop,c("Data_Source_Name_Display")] <- paste(as.character(FMLE_StudyStates_sepCodes[,c("Dataset")]),as.character(FMLEdata_Parameter_MetaData$Code),sep = " ") #"Dataset" 

# "Data_Source_Name_Short"
input_mat1[row_start:row_stop,c("Data_Source_Name_Short")] <-  paste(as.character(FMLE_StudyStates_sepCodes[,c("Dataset")]),as.character(FMLEdata_Parameter_MetaData$Code),sep = "")#as.character(FMLE_StudyStates_sepCodes[,c("Dataset")])# "Dataset" 
...
# input color for plotting this data source (totally arbitrary choice of color)
input_mat1[row_start:row_stop,c("PlottingColor")] <- "orange"  #"lightsalmon4" 
...
FMLEdata_all_states <- read.csv(file.path(FMLE.directory,this_source_file), header = T, sep = ",",blank.lines.skip = F)
# load parameter description
FMLEdata_Parameter_MetaData <- read.csv(file.path(FMLE.directory,this_source_file_full), header = T, sep = ",",blank.lines.skip = T,nrows = 1,skip = 209)
rm(this_source_file_full)
...
# "Data_Source_Name_Display"
input_mat1[row_start:row_stop,c("Data_Source_Name_Display")] <- paste(as.character(FMLE_StudyStates_sepCodes[,c("Dataset")]),as.character(FMLEdata_Parameter_MetaData$Code),"III1",sep = " ")#paste(as.character(FMLE_StudyStates_sepCodes[,c("Dataset")]),as.character(FMLEdata_Parameter_MetaData$Code),sep = " ") #"Dataset" 

# "Data_Source_Name_Short"
input_mat1[row_start:row_stop,c("Data_Source_Name_Short")] <-  paste(as.character(FMLE_StudyStates_sepCodes[,c("Dataset")]),as.character(FMLEdata_Parameter_MetaData$Code),"III1",sep = "")#paste(as.character(FMLE_StudyStates_sepCodes[,c("Dataset")]),as.character(FMLEdata_Parameter_MetaData$Code),sep = "")#as.character(FMLE_StudyStates_sepCodes[,c("Dataset")])# "Dataset" 
...
# input color for plotting this data source (totally arbitrary choice of color)
input_mat1[row_start:row_stop,c("PlottingColor")] <- "seagreen"  #"orange"  #"lightsalmon4" 
...
#### Fill in data from Federal Land Managers - IMPROVE RHR III 'second param' (88101) ######################
data_source_counter <- data_source_counter+1
...
### Load location meta-data #
    ...
    this_meta_data_file <- "Missing_Site_Info_for_Ellen-6-4-18.csv"
    second_meta_data_file <- read.csv(file.path(CARB.directory,this_meta_data_file), header = T, sep = ",")
    rm(this_meta_data_file)
...
### compare location info between CARB_data and CARB_meta_data
    ...
    all_sites_step <- unique(c(CARB_data$AQS.Site.ID,CARB_meta_data$AQS.ID,second_meta_data_file$x))
    all_sites <- all_sites_step[which(!is.na(all_sites_step))]
    print(all_sites)
    rm(all_sites_step)
    
    N_CARB_EPACodes <- length(unique(CARB_data$AQS.Site.ID))
    compare_loc_CARBs_header <- c("Site.ID","Burton.Lat","Burton.Lon","Second.Burton.Lat","Second.Burton.Lon","Lat.w.PM25","Lon.w.PM25")
    N_compare_col <- length(compare_loc_CARBs_header)
    compare_loc_CARBs <- data.frame(matrix(NA,nrow = length(all_sites),ncol = N_compare_col))
    names(compare_loc_CARBs) <- compare_loc_CARBs_header
    ...
    for (i_Burton_row in 1:dim(CARB_meta_data)[1]) { # cycle through sites in first file from Burton and fill in location data
      ...
      print(this_EPA_code)
      which_compare <- which(compare_loc_CARBs$Site.ID==this_EPA_code)
      print(which_compare)
      #compare_loc_CARBs[which_compare,c("Burton.Lat","Burton.Lon")] <- CARB_meta_data[i_Burton_row,c("Latitude","Longitude")]
      compare_loc_CARBs[which_compare,c("Burton.Lat")] <- as.character(CARB_meta_data[i_Burton_row,c("Latitude")])
      compare_loc_CARBs[which_compare,c("Burton.Lon")] <- as.character(CARB_meta_data[i_Burton_row,c("Longitude")])
    ...
    for (i_Burton_row in 1:dim(second_meta_data_file)[1]) { # cycle through sites in second file from Burton and fill in location data
      ...
      if (nchar(meta2_this_lat_lon)<=1) { # second Burton data does not have location info
        compare_loc_CARBs[which_compare,c("Second.Burton.Lat")] <- NA
        compare_loc_CARBs[which_compare,c("Second.Burton.Lon")] <- NA
      } else { # there is data
        ...
        print(this_lon)
        compare_loc_CARBs[which_compare,c("Second.Burton.Lat")] <- this_lat
        compare_loc_CARBs[which_compare,c("Second.Burton.Lon")] <- this_lon
    ...
    which_missing_Burton <- which(is.na(compare_loc_CARBs$Burton.Lat) & is.na(compare_loc_CARBs$Second.Burton.Lat))
...
# Split FMLE EPACode into State_Code, County_Code and Site_Num for IMPROVE data and put them into input_mat1
N_CARB_EPACodes <- length(unique(CARB_data$AQS.Site.ID))
CARB_EPACode_header <-  c("EPACode","StateCode","CountyCode","SiteNum","ARB.Site.Name","Latitude","Longitude","Elevation..m.","ARB.Site..","State","County","Site.ID","Address","City","Zip.Code","AQMIS.Code","LocataionDataSource")
...
for (this_row in 1:N_CARB_EPACodes) { # cycle through each row in CARB data to determine state code, county code, and site num and put into input_mat1
  ...
  CARB_EPACode[this_row,c("AQMIS.Code")] <- as.character(CARB_meta_data[which_meta_row,c("AQMIS.Code")])
    ...
    which_compare_loc <- which(as.character(compare_loc_CARBs$Site.ID)==this_EPACode)
    stop("pick up code here")
    if (!is.na(compare_loc_CARBs[which_compare_loc,c("Burton.Lat")])) {stop("check code")}
    if (!is.na(compare_loc_CARBs[which_compare_loc,c("Second.Burton.Lat")])) {
      print("Location info in second Burton File")
      stop("pick up code here")
    } else if (!is.na(compare_loc_CARBs[which_compare_loc,c("Lat.w.PM25")])) {
      print("using data in origninal file from CARB")
      stop("pick up writing code with orig CARB location info")
      CARB_EPACode[this_row,c("Latitude")] <- as.character(compare_loc_CARBs[which_compare_loc,c("Lat.w.PM25")])
      CARB_EPACode[this_row,c("Longitude")] <- as.character(compare_loc_CARBs[which_compare_loc,c("Lon.w.PM25")])
...
for (this_row in 1:dim(CARB_EPACode)[1]) { # put columns of state code, county code, and site number into CARB_StudyStates_sepCodes
  ...
  CARB_data[rows_of_interest,c("Burton_Lat")] <- CARB_EPACode[this_row,c("Latitude")]
  CARB_data[rows_of_interest,c("Burton_Lon")] <- CARB_EPACode[this_row,c("Longitude")]
...
# just using location info from Eric Burton
input_mat1[row_start:row_stop,c("PM2.5_Lat")] <- CARB_data[,c("Burton_Lat")]
input_mat1[row_start:row_stop,c("PM2.5_Lon")] <- CARB_data[,c("Burton_Lon")]
...
# print('pull in new Utah PM2.5 data')
    ...
    # input 'Datum' into input_mat1
    input_mat1[row_start:row_stop,c("Datum")] <- Datum_used
    #this_col <- 'Datum'
...
# #### COMMENTED Pull in Nevada PM2.5 data ####
# All the data that is recommended for use is already in the EPA data (see email from Ellen on May 29, 2018)
    #     
#     # NV site location
...
#         print(this_source_file)
#         
#         NVDEQ_data<-read.csv(file.path(NVDEQ.directory,this_source_file),header=TRUE,skip = 1) # load the AQS file
#         
