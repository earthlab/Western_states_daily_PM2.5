...
Replace_LatLon_Reprojected.fn <- function(input_mat2,link_projections,Reprojected_datum) { # start function 
...
for (this_row_i in 1:dim(link_projections)[1]) { # cycle through rows of the reprojected data and replace values in input_mat3
  ...
  which_this_lat_lon <- which(input_mat3$PM2.5_Lat == link_projections[this_row_i,c("Old.Lat")] &
                              input_mat3$PM2.5_Lon == link_projections[this_row_i,c("Old_Lon")] &
...
which(input_mat1$PM2.5_Lat==40.44711217)
which(input_mat1$PM2.5_Lat==40.447)

which(input_mat1$Source_File==Not_replaced[1,c("Source_File")])
