...
class GASP:
    ...
    def sort_data(self, origpath, outpath, item):  # Write valid lat, lon, aod values to file with local UTC name (requires time conversion)
        ...
        LL_df = pd.DataFrame.from_csv('C:\\Users\\ellen\\OneDrive\\MyDocs\\Earth Lab Internship\\Spatial_Processing\\test_data\\GASP\\' 
                       + "lat_lon_tzid_lookup.txt") #need to put this in the correct folder before starting
        
        aod_table = pd.read_table(origpath + item, header= None)
        # print(aod_table.head())
        aod_df = pd.DataFrame(aod_table)
        #print(aod_df)
        ...
        exists = os.path.isfile(outpath + "GASP_" + yesteryear + "." + str(yesterday) + ".csv")
        yesterfile = outpath + "GASP_" + yesteryear + "." + str(yesterday) + ".csv"
        todayfile = outpath + "GASP_" + year + "." + str(today) + ".txt"
        
        if(exists):
            with open(yesterfile, 'a') as f:
                yesterday_df.DataFrame.to_csv(outpath + "GASP_" + yesteryear + "." + str(yesterday) + ".csv", header=False)
        else:
            yesterday_df.DataFrame.to_csv(yesterfile, header=False)
        
        today_df.DataFrame.to_csv(todayfile, header=False)
...
#         # Step1
        outpath1 = 'C:/Users/ellen/OneDrive/MyDocs/Earth Lab Internship/Spatial_Processing/test_data/GASP/test_results/step1/'
#         pool = multiprocessing.Pool()
...
#         pool.join()
        ...
        # Step2
        outpath2 = 'C:/Users/ellen/OneDrive/MyDocs/Earth Lab Internship/Spatial_Processing/test_data/GASP/test_results/step2/'
        pool = multiprocessing.Pool()
...
#             pool.apply_async(self.to_shapefile, [outpath3, outpath4, item])
#         pool.close()
#         pool.join()
        
#         # Step5
#         outpath5 = 'C:/Users/ellen/OneDrive/MyDocs/Earth Lab Internship/Spatial_Processing/test_data/GASP/test_results/step5/'
#         pool = multiprocessing.Pool()
#         for item in os.listdir(outpath4):
#             pool.apply_async(self.to_gdal_raster, [outpath4, outpath5, item])
#         pool.close()
#         pool.join()
