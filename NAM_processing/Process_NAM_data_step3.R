...
new_file_list <- unlist(lapply(this_file_list, function(x){ # start lapply and start defining function used in lapply
  ...
  this_data <- read.csv(file.path(define_file_paths.fn("ProcessedData.directory"),sub_folder,x)) # open data file
...
setwd(file.path(define_file_paths.fn("ProcessedData.directory"),sub_folder)) # change working directory so the next step will work
...
# write data to file
write.csv(Step3_NAM_data,file = file.path(define_file_paths.fn("ProcessedData.directory"),sub_folder,paste(output_file_name,".csv",sep = "")),row.names = FALSE)
