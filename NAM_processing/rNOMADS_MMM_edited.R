# CheckNOMADSArchive_MMM function is just a slightly modified version of https://github.com/cran/rNOMADS/blob/master/R/GetArchiveGrib.R
CheckNOMADSArchive_MMM <- function(abbrev, model.date = NULL) { 
  ...
  if(is.null(model.date)) { #Check all available dates
    ...
    if(RCurl::url.exists(check.url)) {
      model.list_step <- grep(".grb", #grep("grb\\d?$",  # MMM edited
                              LinkExtractor(check.url),
                              value = TRUE)
      n_chars <- nchar(abbrev)+22+5
      this_model_list <- unique(unlist(lapply(1:length(model.list_step), function(X){
        step1 <- substr(model.list_step,1,n_chars)
        gsub(">","",step1)
      })))
      #model.list <- append(
      #  model.list, 
      #  grep(".grb", #grep("grb\\d?$",  # MMM edited
      #       LinkExtractor(check.url),
      #       value = TRUE))       
      model.list <- append(
        model.list, 
...
ArchiveGribGrab_MMM <- function(abbrev, model.date, model.run, preds, local.dir = NULL, file.names = NULL,
  ...
  for(k in 1:length(preds)) {
    ...
    #    paste0("^.*", file.part, "><")),
    #  "><", "")
    grb.urls <- unlist(lapply(1:length(preds), function(x) {
      paste0(download.url,link.list[grepl(file.part, link.list)])
    }))
...
# LinkExtractor <- function(check.url,abbrev)#function(url) { # minor modifications made to https://github.com/cran/rNOMADS/blob/master/R/RNomadsTools.R # MMM edited
...
#   
#   html.tmp <- readLines(check.url, warn = FALSE)#html.tmp <- readLines(url, warn = FALSE) # MMM edited
#   
