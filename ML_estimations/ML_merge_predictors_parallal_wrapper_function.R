ML_merge_predictors_parallal_wrapper.fn <- function(task_counter){ #, input_header, ProcessedData.directory, AQSData.directory, FireCache.directory, UintahData.directory) {
  ...
  if (task_counter == 1) {
    ...
    study_start_date <- as.Date("20080701",format="%Y%m%d") # first date in study period
    study_stop_date  <- as.Date("20080731",format="%Y%m%d") # last date in study period
    
    Merged_input_file <- merge_predictors.fn(predictand_data = predictand_data_full, predictand_col = predictand_col, 
                                             ...
                                             task_counter = task_counter, study_start_date = study_stop_date, study_stop_date = study_stop_date)
