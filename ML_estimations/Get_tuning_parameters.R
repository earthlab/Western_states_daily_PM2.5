...
sink("C:\\Users\\elco2649\\Documents\\Machine Learning\\Get_tuning_parameters_and_compare_algs.txt")
...
for(SS in Subsets){
  ...
  for(seed in c(321, 654, 987)){
    ...
    ranger_end<- Sys.time()
    print(paste("ranger:", ranger_end - ranger_start))
    ...
    for(A in other_algs){
      ...
      model_end<- Sys.time()
      print(paste0(A, ": ", model_end - model_start))
    }
