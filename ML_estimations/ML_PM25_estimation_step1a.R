...
plot_variables <- c("MAIAC_AOD", "UGRD.10.m.above.ground")
...
file_sub_label <- paste("ML_input_report_",substr(this_source_file, 1, (nchar(this_source_file)-4)),"pdfs",sep = "") # file partial name, decide whether to include date in file name
print(file_sub_label)
title_string_partial <- "ML Inputs Time Series"
this_image_format <- "pdf"
...
df_report.fn(df = PM25_obs_shuffled, cols_interest = c(col_name_interest, plot_variables), x_axis_var = "Date", output.directory = output.directory,
             ...
             LatexFileName = LatexFileName, SinkFileName = NA, image_format = this_image_format)
  
#title_string_partial <- "ML Inputs Plot against PM2.5"
title_string_partial <- paste(" vs. ",col_name_interest,sep = "")
...
#,predictor_variables
df_report.fn(df = PM25_obs_shuffled, cols_interest = plot_variables, x_axis_var = col_name_interest, output.directory = output.directory,
             output.directory.short = output.directory.short, file_sub_label = file_sub_label, title_string_partial = title_string_partial, plot_color = "black",
             LatexFileName = LatexFileName, SinkFileName = NA, image_format = this_image_format)
...
print(paste("ML_PM25_estimation_step1a.R completed at",Sys.time(),sep = " "))
# stop the timer
